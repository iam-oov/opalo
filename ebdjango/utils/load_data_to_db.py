...
# leer csv
path_file_historicos_mercados = os.path.abspath(os.path.join(
    DJANGO_DIR, '../data/csv/01TablasSQL_historico_mercados.csv'))
path_file_historicos_tasas_gen = os.path.abspath(os.path.join(
...
data_mercados.columns = data_mercados.columns.str.replace(' ','_')

# lowercase in names columns
data_mercados.columns = data_mercados.columns.str.lower()

# rename 
data_mercados = data_mercados.rename({'global': 'global_oh'}, axis=1)
data_mercados = data_mercados.replace(to_replace={"[NULL]": 0})


for row in data_mercados.itertuples():
    obj = mod_core.create_or_update_market(
        row.pk_control,
        row.fecha,
        ...
        expectativa=strings.convert_bool(row.expectativa.replace('"', '')),
