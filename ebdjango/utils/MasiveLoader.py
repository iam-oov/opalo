...
class MasiveLoader:
    ...
    def insert_or_update_marker(self, from_path_file, search_field):
        df = pd.read_csv(from_path_file)
        df = strings.clean_dataframe(df)

        for row in df.itertuples():
            obj = mod_core.create_or_update_obj(
                self.model,
                getattr(row, search_field),
                row.fecha,
                ...
                deuda_internacional_d_soberanos=strings.convert_float(
                    row.deuda_internacional_d_soberanos),
                deuda_internacional_d_crédito=strings.convert_float(
                    row.deuda_internacional_d_crédito),
                ...
                acciones_intl_asia=strings.convert_float(
                    row.acciones_intl_asia),
                global_oh=strings.convert_float(row.global_oh),
                acciones_emergentes=strings.convert_float(
                    row.acciones_emergentes),
                mex_cds_usd_sr_5y_d14_corp=strings.convert_float(
                    row.mex_cds_usd_sr_5y_d14_corp),
                brazil_cds_usd_sr_5y_d14_corp=strings.convert_float(
                    row.brazil_cds_usd_sr_5y_d14_corp),
                colom_cds_usd_sr_5y_d14_corp=strings.convert_float(
                    row.colom_cds_usd_sr_5y_d14_corp),
                chile_cds_usd_sr_5y_d14_corp=strings.convert_float(
                    row.chile_cds_usd_sr_5y_d14_corp),
                ...
                emerging_markets_currency=strings.convert_float(
                    row.emerging_markets_currency),
                ...
                us_federal_funds_effective_rat=strings.convert_float(
                    row.us_federal_funds_effective_rat),
                us_generic_govt_1_month_yield=strings.convert_float(
                    row.us_generic_govt_1_month_yield),
                us_3m=strings.convert_float(row.us_3m),
                us_generic_govt_6_month_yield=strings.convert_float(
                    row.us_generic_govt_6_month_yield),
                us_generic_govt_12_month_yield=strings.convert_float(
                    row.us_generic_govt_12_month_yield),
                us_generic_govt_2_year_yield=strings.convert_float(
                    row.us_generic_govt_2_year_yield),
                usd_libor_3m=strings.convert_float(row.usd_libor_3m),
                german_3m=strings.convert_float(row.german_3m),
                german_treasury_bill_6m=strings.convert_float(
                    row.german_treasury_bill_6m),
                german_government_bills_1_yr=strings.convert_float(
                    row.german_government_bills_1_yr),
                ftse_mib=strings.convert_float(row.ftse_mib),
                expectativa=strings.convert_bool(
                    str(row.expectativa).replace('"', '').lower()),
                liquidez_local=strings.convert_float(row.liquidez_local),
            )

    def insert_or_update_rate_gen(self, from_path_file, search_field):
        df = pd.read_csv(from_path_file)
        df = strings.clean_dataframe(df)
